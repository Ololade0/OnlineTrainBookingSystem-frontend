...
//     setLoading(true);
//     try {
//       const queryParams = `page=${page}&size=${size}${searchQuery ? `&query=${encodeURIComponent(searchQuery)}` : ""}`;
//       const response = await fetch(
//         `${API_BASE}/user/getAllNonUserAccounts?${queryParams}`,
...
//       fetchStaffs();
//     }
//   }, [auth, page, showForm, fetchStaffs]);
...
export default function StaffList() {
  ...
  const fetchStaffs = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/user/searchUsers?${queryParams}`, {
        headers: { Authorization: `Bearer ${auth?.token}` },
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.message || data.status?.description || "Failed to fetch staffs");
  ...
  return (
    <div className={styles.container}>
      <UserDetailsModal user={selectedUser} loading={loadingUser} onClose={() => setSelectedUser(null)} />

      {!showForm ? (
        <>
          <div className={styles.headerRow}>
            <h2 className={styles.title}>Staff Accounts</h2>
            <button className={styles.addButton} onClick={() => setShowForm(true)}>+ Add Staff</button>
          </div>
